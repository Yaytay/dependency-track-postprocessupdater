...
func (c *Client) ApplyPostProcessing(ctx context.Context, projectUUID string, tags []string, suppressions []model.Suppression) error {
	...
	if len(suppressions) > 0 {
		if err := c.uploadSuppressions(ctx, projectUUID, suppressions); err != nil {
			return err
		}
	}
	return nil
...
func (c *Client) addTags(ctx context.Context, projectUUID string, tags []string) error {
	payload := make([]TagRequest, 0, len(tags))
	for _, t := range tags {
		t = strings.TrimSpace(t)
		if t != "" {
			payload = append(payload, TagRequest{Name: t})
		}
	}
	if len(payload) == 0 {
		return nil
	...
	u, err := url.Parse(c.baseURL + "/api/v1/project/" + projectUUID + "/tags")
	if err != nil {
		return err
	}
...
func (c *Client) uploadSuppressions(ctx context.Context, projectUUID string, suppressions []model.Suppression) error {
	body, err := json.Marshal(suppressions)
	if err != nil {
		return err
	}

	u, err := url.Parse(c.baseURL + "/api/v1/project/" + projectUUID + "/suppressions")
	if err != nil {
		return err
	}
	req, err := http.NewRequestWithContext(ctx, http.MethodPut, u.String(), bytes.NewReader(body))
